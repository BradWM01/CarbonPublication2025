...
\todo{When I read this, I begin to question it, because when we stitch it
without the blending parameter, we see significant variation between
micrographs. So, i have said that the linear intensity blending parameter had no
discenable variation between different types of belidng, but of course that is
not the same as saying that there is no difference between no blending and
blending. Will check this on Mon 9th}
...
\todo{Interestingly, the unimodality is be a positive indication as to the
accuracy of the  3.14-3.38\% 2.30-3.66\% channel porosities. This shows that
porosity really is a small proportion of sample surface, but of course it is
just a definition thing. Maybe that porosity is all the values less than white,
distributed a little in each bin from gray to black}
  ...
  \todo{For future work, we should try a unimodal thresholding algo (which I did
  not know existed) developed by Rosin.
...
size distribution generated by SEM. For example, the modelling would I think be
distorted completely by saying that 3\% of the sample surface is made up of
pores of 1.12-25 µm diameter, when that number may actually represent all pores
above 1.12 µm, and it is just fragmenting the pores. Being unsure about the
exact diameter range of pores covered by SEM must surely be making the modelling
less accurate, and the pore size distribution more skewed.}
